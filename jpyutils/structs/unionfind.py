...
Object must be Python hashable.'''
        if obj not in self.objs_to_num:
            obj_num = len(self.objs_to_num)
            self.num_weights[obj_num] = 1
            self.objs_to_num[obj] = obj_num
            self.num_to_objs[obj_num] = obj
            self.parent_pointers[obj_num] = obj_num
            return obj
        stk = [self.objs_to_num[obj]]
...
If either or both objects are unknown, will make them known, and combine them.'''
        ...
        o2p = self.find(obj2)
        if o1p != o2p:
            on1 = self.objs_to_num[o1p]
            on2 = self.objs_to_num[o2p]
...
Insert a sequence of objects into the structure.  All must be Python hashable.'''
        for obj in objs:
            self.find(obj);
    def get_roots(self):
        return [self.num_to_objs[k] for k in self.num_weights.keys()]
    def __len__(self):
        '''\
...
using find.'''
        sets = {}
        for i in xrange(len(self.objs_to_num)):
            sets[i] = []
        for i in self.objs_to_num:
            sets[self.objs_to_num[self.find(i)]].append(i)
        out = []
